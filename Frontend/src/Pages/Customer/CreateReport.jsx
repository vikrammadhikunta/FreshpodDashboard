// pages/admin/AdminReports.jsx - FIXED with correct axiosInstance usage
...
const AdminReports = () => {
  ...
  const fetchReports = async (page = 1) => {
    ...
    try {
      ...
      // ✅ FIXED: Using axiosInstance directly like customer create report
  ...
  const handleSolveReport = async (reportId) => {
    try {
      // ✅ FIXED: Using axiosInstance directly
      ...
      alert('❌ Failed to solve report: ' + (err.response?.data?.message || 'Unknown error'));
    }
  };
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{statistics.solved}</p>
            </div>
              ...
              {loading ? (
                ...
                reports.map((report) => (
                  <tr key={report.reportId} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">{report.customer.name}</div>
                      <div className="text-sm text-gray-500">{report.customer.phone}</div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-gray-400">{report.timeElapsed} ago</div>
                    </td>
                    <td className="px-6 py-4">
